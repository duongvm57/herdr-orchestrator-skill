...
def render_document(manifest: dict[str, Any]) -> str:
    ...
    for group in manifest["groups"]:
        invariant_ids = ", ".join(f"`{item['id']}`" for item in group["invariants"])
        lines.extend(["", f"### {group['title']}", "", f"- Verification: **{group['verification']}**", f"- Invariants: {invariant_ids}", "", textwrap.fill(group["scenario"], width=88, break_long_words=False)])
