...
class LiveEvalTests(unittest.TestCase):
    ...
    def test_live_prompt_contains_no_grader_truth(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("Before your turn settles", prompt)
            self.assertIn("Runner preflight is already complete", prompt)
            self.assertIn("fresh HOME and CODEX_HOME", prompt)
            self.assertIn("HOME=$HOME", prompt)
            self.assertIn("evaluation-evidence-template.json", prompt)
            self.assertIn("./evaluation-evidence.json", prompt)
            self.assertIn("do not place that index under evidence/", prompt)
            self.assertIn("never a JSON list", prompt)
            self.assertIn("Never list this Lead", prompt)
            self.assertIn("sha256sum", prompt)
            self.assertIn("peer-assignment-template.json", prompt)
            self.assertIn("owner is never the Lead", prompt)
            self.assertIn("Choose each distinct Peer name before creating its Assignment", prompt)
            self.assertIn('authority "write"', prompt)
            self.assertIn("do not improvise a Peer prompt", prompt)
            self.assertIn("exactly assignment_id, outcome, evidence, impact, and need", prompt)
            self.assertIn("canonical semantic outcome", prompt)
            self.assertIn('"assignment_sha256"', prompt)
            self.assertIn("# Lead role/profile", prompt)
            self.assertIn("# Project Lead", prompt)
            self.assertIn("# Full Workspace Protocol", prompt)
            self.assertIn("# Consumer protocol", prompt)
            self.assertIn("# Configured Peer recipes", prompt)
            self.assertIn("Bounded Luna Peer for isolated evaluation work.", prompt)
            ...
            self.assertIn("nonempty topology_rationale", coupled_prompt)
            self.assertNotIn("create one Peer Assignment whose owned_scope", multi_scope_prompt)
            self.assertNotIn("create one Peer Assignment whose owned_scope", coupled_prompt)

    def test_zero_peer_prompt_does_not_require_peer_creation(self) -> None:
        ...
        self.assertIn("requires zero Peer agents", prompt)
        self.assertIn("do not create a Peer Assignment, Peer pane, Peer agent, or Peer handback", prompt)
        self.assertNotIn("launch the required named Peer once", prompt)
